...
// Return the iterator of the IfRegion Op, this is the last one of the IfRegion
// of the segment or subgroup of the IfRegion group.
// For example, we have the following sequence of IfRegions
// `````
//      1          2          3         4           5
// IfRegionA, IfRegionA, IfRegionA, IfRegion A, IfRegionA
// `````
// The first three IfRegionA are in one group and the last two are in another
// group. Then when we call FindSubgroupLasrIfOpIter for the first group, it
// will return iterator of the 3th IfRegionA.
// In the same way, when we call it for the second group, it will return
...
IfOpIter FindSubgroupLastIfOpIter(
  ...
  auto get_middle_if_ops = [&](TF::IfRegionOp intermediate_op) {
    ...
    for (auto& op : all_ops) {
      downstream_if_ops.insert(op);
    }
    return downstream_if_ops;
  };
  ...
  for (auto it = std::next(current); it != std::next(potential_last); it++) {
    ...
    downstream_if_ops = get_middle_if_ops(second_if_op);
    if (SafeToMerge(second_if_op, first_if_op, downstream_if_ops, analysis)) {
      continue;
    }
