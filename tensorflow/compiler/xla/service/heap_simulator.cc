...
GlobalDecreasingSizeBestFitHeap<BufferType>::FindChunkCandidate(
  ...
  auto subtract_used_chunks = [&](const std::vector<Chunk>& chunks) {
    for (const Chunk& chunk : chunks) {
      // Find the free chunks containing the start and end of the used chunk.
      auto it_start = free_chunks.lower_bound(chunk.offset);
      auto it_end = free_chunks.lower_bound(chunk.chunk_end());
      if (it_end == free_chunks.end()) it_end = free_chunks.begin();
      ...
      if (it_start != free_chunks.end()) {
        if (chunk.offset - it_start->first >= buffer_interval.size) {
          it_start->second = std::min(it_start->second, chunk.offset);
        } else {
      ...
      int64_t chunk_end_aligned = RoundUpTo(chunk.chunk_end(), alignment_);
  ...
  for (const BufferType* colocation :
    ...
    BufferInterval interval = buffer_intervals_.at(colocation);
  ...
  auto it = free_chunks.lower_bound(preferred_offset);
  if (it == free_chunks.end() || (it->second < chunk.chunk_end())) {
    // Otherwise, find the smallest free chunk. We ensure above that all of the
    // free chunks are large enough to store the buffer.
    chunk.offset = absl::c_min_element(free_chunks, [](auto a, auto b) {
                     return (a.second - a.first) < (b.second - b.first);
                   })->first;
  }

  return {chunk, std::max(result_.heap_size, chunk.chunk_end())};
}
